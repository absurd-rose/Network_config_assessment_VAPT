...
#         keywords = extract_keywords(sw.get("name", ""))
#         for kw in keywords:
#             cves = query_nvd(kw)
#             for cve in cves:
...
#                 matched_cves.append({
#                     "timestamp": sw.get("scan", {}).get("time", "Unknown"),
...
def generate_report(agent_id):
    ...
    # Query NVD once per unique keyword
    # for kw, software_refs in keyword_to_software.items():
    #     cves = query_nvd(kw)
    #     for cve in cves:
    #         if cve["cve_id"] in seen_cves:
    #             continue
...
# @app.route('/api/dashboard/<int:device_id>', methods=['GET'])
# def get_dashboard(device_id: int):
#     # Call the existing generate_report function
#     modified_device_id = "00" + str(device_id)
#     print(modified_device_id)
#     print(modified_device_id)
#     report, status = generate_report(modified_device_id)
#     return jsonify(report), status
